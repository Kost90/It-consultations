...
const LegalServices = () => {
    ...
    const filteredServiceTile = data.map((item)=>{
        if(item.type==="Financial-Consultancy" && (selectedTrainer === "" || item.professional === selectedTrainer))
    ...
    const deliverDropdown = ()=> {
        const doubleCons = data.map((item)=>{
            if(item.type==="Financial-Consultancy")
  ...
  return (
    <div>
        ...
        <div className="headline">Financial-Consultancy</div>
